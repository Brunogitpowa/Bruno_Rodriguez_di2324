from PySide6.QtWidgets import (QApplication, QMessageBox, QMainWindow, QWidget, QVBoxLayout, QPushButton, QHBoxLayout, QLabel, QLineEdit)
...
class VentanaPrincipal(QMainWindow):
    ...
    def iniciar_sesion(self):
        if self.usuario_text.text() == 'admin' and self.contrasena_text.text() == 'admin':
            QMessageBox.information(self, "Inicio de sesión", "Inicio de sesión correcto", QMessageBox.Ok)
        else:
            self.frase.setStyleSheet('color: red')
            QMessageBox.warning(self, "Inicio de sesión", "Usuario o contraseña incorrectos", QMessageBox.Ok)
