...
from PySide6.QtWidgets import QApplication, QTextEdit, QMainWindow, QToolBar, QFileDialog
...
class VentanaPrincipal(QMainWindow):
    ...
    def abrirArchivo(self):
        ruta_al_archivo, _ = QFileDialog.getOpenFileName(self, "Abrir archivo")
        if (ruta_al_archivo):
            archivo = QFile(ruta_al_archivo)
            if archivo.open(QFile.ReadOnly | QFile.Text):
                texto = archivo.readAll().data().decode()
    ...
    def guardarArchivo(self):
        ruta_al_archivo, _ = QFileDialog.getSaveFileName(self, "Guardar archivo")
        if ruta_al_archivo:
            archivo = QFile(ruta_al_archivo)
            if archivo.open(QFile.WriteOnly):
                archivo.write(self.editor.toPlainText().encode())
                archivo.flush()
                archivo.close() 
